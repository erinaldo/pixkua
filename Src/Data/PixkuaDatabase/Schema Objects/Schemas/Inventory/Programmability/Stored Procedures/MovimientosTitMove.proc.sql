CREATE PROCEDURE [Inventory].[MovimientosTitMove]
	@Id as char(18),
	@IdAlmacen as int,
...
BEGIN
	...
	BEGIN		
		...
		BEGIN
			SET @ErrorMessage = N'No existe la cantidad suficiente del Producto "' + RTRIM(LTRIM(@Codigo)) + '-' + RTRIM(LTRIM(@Articulo)) + '" en el almacén "' + @Almacen + '" - (Existente:' + RTRIM(LTRIM(IsNull(@Disponible,0))) + ' --> Solicitada:'  + RTRIM(LTRIM(IsNull(@CantidadBase,0))) + ')'
		...
		EXEC Inventory.InventarioUPD @IdAlmacen=@IdAlmacen, @IdArticuloStock=@IdArticuloStock, @Disponible=@CantidadBase,
			@Apartado=0, @Transito=0, @Direccion=@Direccion, @UserUpdate=@UserUpdate, @LastUpdate=@Hora, @PCUpdate=@PCUpdate
	...
	UPDATE Inventory.MovimientosTit SET HoraMovto=@Hora, HoraAplicado=@Hora, IdCorte=@IdCorte
