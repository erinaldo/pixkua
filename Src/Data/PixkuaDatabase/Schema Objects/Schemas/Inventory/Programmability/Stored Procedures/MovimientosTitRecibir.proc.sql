CREATE PROCEDURE [Inventory].[MovimientosTitRecibir]
	@Id as char(18),
	@IdAlmacen as int,
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			SET @ErrorMessage = N'No existe el producto en transito suficiente del Producto "' + RTRIM(LTRIM(@Codigo)) + '-' + RTRIM(LTRIM(@Articulo)) + '" en el almacén "' + @Almacen + '" - (' + RTRIM(LTRIM(IsNull(@Transito, 0))) + ')'
			RAISERROR (@ErrorMessage,18,1)
			RETURN
		END
		-->Actualizar inventario
		EXEC Inventory.InventarioUPD @IdAlmacen=@IdAlmacen, @IdArticuloStock=@IdArticuloStock, @Disponible=0,
			@Apartado=0, @Transito=@CantidadBase, @Direccion=-1, @UserUpdate=@UserUpdate, @LastUpdate=@Hora, @PCUpdate=@PCUpdate

		--Actualizar saldos iniciales y finales
		UPDATE Inventory.MovimientosDet 
		SET CantidadBase=@CantidadBase,
			TransitoInicial=@Transito, TransitoFinal=@Transito + (@CantidadBase * @Direccion)			
	...
	UPDATE Inventory.MovimientosTit SET HoraAplicado=getdate()
