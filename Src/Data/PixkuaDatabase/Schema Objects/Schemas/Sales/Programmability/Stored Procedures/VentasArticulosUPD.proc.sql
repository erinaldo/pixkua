CREATE PROCEDURE [Sales].[VentasArticulosUPD]	
...
AS
	...
	SELECT @IdEstCom=A.IdEstCom
	FROM Sales.Articulos A JOIN Sales.ArticulosPresentaciones AP
		ON A.IdArticulo=AP.IdArticulo
	WHERE AP.IdArticuloStock=@IdArticuloStock
	
		
	IF EXISTS (SELECT IdVentaTit FROM Sales.VentasDet WHERE IdVentaTit=@IdVentaTit and Fecha=@Fecha AND IdArticuloStock=@IdArticuloStock AND IdServicio=@IdServicio)
		UPDATE Sales.VentasDet SET Codigo=@Codigo, Descripcion=@Descripcion, Cantidad=@Cantidad, 
			...
			MontoImpRet4=@MontoImpRet4, MontoImpRet5=@MontoImpRet5, Flete=@Flete, PorcComision=@PorcComision,
			MontoComision=@MontoComision
		WHERE IdVentaTit=@IdVentaTit and Fecha=@Fecha 
			AND IdArticuloStock=@IdArticuloStock AND IdServicio=@IdServicio	
	ELSE
