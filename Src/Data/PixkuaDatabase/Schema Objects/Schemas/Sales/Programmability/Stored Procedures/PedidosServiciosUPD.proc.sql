CREATE PROCEDURE [Sales].[PedidosServiciosUPD]
	@IdPedidoDet as int=0 out,
...
AS
	...
	SELECT @Fecha=Documents.fxDocumentDate(@IdPedidoTit)
			
	IF EXISTS (SELECT IdPedidoTit FROM Sales.PedidosDet WHERE IdPedidoTit=@IdPedidoTit and Fecha=@Fecha AND IdArticuloStock=@IdArticuloStock AND IdServicio=@IdServicio AND IdPedidoDet=@IdPedidoDet)
		UPDATE Sales.PedidosDet SET Codigo=@Codigo, Descripcion=@Descripcion, Cantidad=@Cantidad, 
		...
		WHERE IdPedidoTit=@IdPedidoTit and Fecha=@Fecha 
			AND IdArticuloStock=@IdArticuloStock AND IdServicio=@IdServicio AND IdPedidoDet=@IdPedidoDet
	
	ELSE
	BEGIN				
		INSERT INTO Sales.PedidosDet(IdPedidoTit, Fecha, IdArticuloStock, IdServicio, Codigo, Descripcion, 
		...
		VALUES (@IdPedidoTit, @Fecha, @IdArticuloStock, @IdServicio, @Codigo, @Descripcion, 
			@Cantidad, 0, @PrecioUnitario, @IdPromocion,
			@PorcDescuento1, @PorcDescuento2, @PorcDescuento3, @PorcDescuento4, @PorcDescuento5, 
			@MontoDesc1, @MontoDesc2, @MontoDesc3, @MontoDesc4, @MontoDesc5, @IdImpuesto1, 
			@IdImpuesto2, @IdImpuesto3, @IdImpuesto4, @IdImpuesto5, @PorcImpTras1, @PorcImpTras2, 
			@PorcImpTras3, @PorcImpTras4, @PorcImpTras5, @MontoImpTras1, @MontoImpTras2, 
