CREATE FUNCTION [Sales].[fxPrecioServicioCliente](@IdServicio as int, @IdSucursal as int, @IdCliente as int, @PrecioFijo as decimal(19,4))
...
BEGIN
	...
	FROM Sales.ServiciosZonasVenta WITH (READUNCOMMITTED)
	WHERE IdZonaVenta=@IdZonaVenta AND IdServicio=@IdServicio
	...
	BEGIN
		...
		SELECT @Precio=Precio FROM Sales.ListasPreciosServicios WITH (READUNCOMMITTED)
	...
	FROM Sales.GruposDescuentoServicios WITH (READUNCOMMITTED)
	WHERE IdGrupoDescuento=@IdGrupoDescuentos AND IdServicio=@IdServicio
