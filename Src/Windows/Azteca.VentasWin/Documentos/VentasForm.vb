...
#Region "Metodos Privados"
    ...
    Private Sub SetInfoAduana(venta As VentaInfo)
        For Each drArticulo As DetalleVentaDS.ArticulosRow In venta.Articulos
            If drArticulo.RowState <> DataRowState.Deleted Then
                drArticulo.NumPedimento = txtNumPedimento.Text
                drArticulo.Aduana = txtAduana.Text
                drArticulo.FechaPedimento = dtpFechaPedimento.Value
            End If
        Next

        For Each drServicio As DetalleVentaDS.ServiciosRow In venta.Servicios
            If drServicio.RowState <> DataRowState.Deleted Then
                drServicio.NumPedimento = txtNumPedimento.Text
                drServicio.Aduana = txtAduana.Text
                drServicio.FechaPedimento = dtpFechaPedimento.Value
            End If
        Next
    End Sub

    Private Sub LoadInfoAduana(venta As VentaInfo)
        If venta.Articulos.Count > 0 Then
            Dim dr As DetalleVentaDS.ArticulosRow = venta.Articulos(0)
            txtNumPedimento.Text = dr.NumPedimento
            txtAduana.Text = dr.Aduana
            If Not dr.IsFechaPedimentoNull Then dtpFechaPedimento.Value = dr.FechaPedimento
        ElseIf venta.Servicios.Count > 0 Then
            Dim dr As DetalleVentaDS.ServiciosRow = venta.Servicios(0)
            txtNumPedimento.Text = dr.NumPedimento
            txtAduana.Text = dr.Aduana
            If Not dr.IsFechaPedimentoNull Then dtpFechaPedimento.Value = dr.FechaPedimento
        End If
...
#Region "Implementacion"
    ...
    Private Sub VentasForm_PrintingItem(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PrintingItem
        ...
        If Venta.Status = WorkflowSteps.Invoice Then
    ...
    Private Sub VentasForm_SavingItem(ByVal sender As Object, ByVal e As Forms.InfoEntryEventArgs) Handles Me.SavingItem
        ...
        With Venta
            ...
            .FolioFacturaExt = txtFolioFacturaExt.Text
            .IdReferencia = txtReferencia.Text
            .IdOrdenTrabajo = OrdenTrabajoSel.Key
    ...
    Protected Overrides Sub OnApplyingItem(ByVal e As Forms.BusinessForms.DocumentStatusEventArgs)
        ...
        If e.IdStatus = WorkflowSteps.Apply Then
            If Venta.CondicionesPago = CondicionesPago.Contado Then
                Dim frmFormaPago As New SelectFormaPagoForm
        ...
        ElseIf e.IdStatus = WorkflowSteps.Invoice Then
            Dim frmMetodoPago As New MetodoPagoSELForm
    ...
    Private Sub txtCliente_InfoLoaded(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            If txtCliente.Key > 0 Then
                ...
                'Verificar si el cliente es de credito o contado
