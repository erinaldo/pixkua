<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="Login.aspx.vb" Inherits="Azteca.Web.Login" EnableEventValidation="false" %>
...
<body>
    ...
    <script src="Scripts/Json/json2.js?<%=ScriptsVersion%>" type="text/javascript"></script>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#dlgSucursal').wijdialog({ autoOpen: false, modal: true, title: "Seleccione sucursal predeterminada", width: 400, height: 200,
                captionButtons:
                {
                    pin: { visible: false },
            ...
            $('#btnSelectSucursal').click(function(e){
                <%= Page.ClientScript.GetPostBackEventReference(btnSelectSucursal, String.Empty) %>;
                e.preventDefault();
            });

            $('#cboEmpresa').change(function() {
                $('#hdnIdEmpresa').val($(this).val());
                CargarSucursales($(this).val());
            });

            $('#cboSucursal').change(function() {
                $('#hdnIdSucursal').val($(this).val());
            });
            ...
            function CargarEmpresas() {
                var url = "/Services/Kernel/Empresa.svc/LoadListByUsuario";
                var data = {'sessionID': Session.SessionID, 'idUsuario': Session.User.Identity }
                $.ajax({
                    type: "POST", url: url, data: JSON.stringify(data), contentType: "application/json; charset=utf-8", dataType: "json",
                    success: function (msg) {
                        Empresas = $.parseJSON(msg);
                        //Llenar combobox de empresas
                        var $cboEmpresa = $('#cboEmpresa');
                        $cboEmpresa.empty();
                        $.each(Empresas, function () {
                            $cboEmpresa.append($('<option></option>').val(this['IdEmpresa']).html(this['Nombre']));
                        });
                        //Verificar que el usuario tenga acceso al menos a una empresa
                        if (Empresas.length > 0){
                            $('#hdnIdEmpresa').val(Empresas[0].IdEmpresa);
                            CargarSucursales(Empresas[0].IdEmpresa);
                        }
                        else {
                            $('#ErrorContainer').show();                        
                            $('#ErrorText').text('El usuario no tiene acceso a ninguna empresa');
                        }
                    },
                    error: function (e) {
                        $('#ErrorContainer').show();
            ...
            function CargarSucursales(idEmpresa) {
                var url = "/Services/Kernel/Sucursal.svc/LoadListByUsuario";
                var data = { 'sessionID': Session.SessionID, 'idEmpresa': idEmpresa, 'idUsuario': Session.User.Identity }
                $.ajax({
                    type: "POST", url: url, data: JSON.stringify(data), contentType: "application/json; charset=utf-8", dataType: "json",
                    success: function (msg) {
                        Sucursales = $.parseJSON(msg);
                        //Llenar combobox de empresas
                        var $cboSucursal = $('#cboSucursal');
                        $cboSucursal.empty();
                        $.each(Sucursales, function () {
                            $cboSucursal.append($('<option></option>').val(this['IdSucursal']).html(this['Nombre']));
                        });
                        $('#hdnIdSucursal').val(Sucursales[0].IdSucursal);
                        //Si el usuario tiene acceso a mas de una empresa/sucursal, permitir que seleccione una como prodeterminada
                        if (Empresas.length > 1 || Sucursales.length > 1)
                            $('#dlgSucursal').wijdialog('open');
                        else {
    ...
    <form id="form1" runat="server">
        ...
        <div id="dlgSucursal">
            <table id="tblSucursal" width="350px">
                <tr>
                ...
                <tr>
                    <td>Sucursal:</td>
                    <td>
                        <asp:DropDownList ID="cboSucursal" runat="server" Width="200px">
                        </asp:DropDownList>
                    </td>
                    <td></td>
                </tr>                
                <tr>
                    <td colspan="2" align="right" valign="bottom" style="height:50px">
                        <asp:Button ID="btnSelectSucursal" runat="server" Text="Ingresar" 
                            CssClass="ActionButton" onclick="btnSelectSucursal_Click"/>
                    </td>
                </tr>
            </table>
