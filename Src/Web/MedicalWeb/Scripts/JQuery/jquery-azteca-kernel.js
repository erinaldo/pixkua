...
function ApplicationInfo() {
    ...
    this.UITheme = "http://code.jquery.com/ui/1.10.3/themes/redmond/jquery-ui.min.css";
}
...
$(document).ready(function () {
    ...
    //    
    $('link.cssTheme').attr('href', App.UITheme);
});
...
CatalogInfo.prototype.MapFields = function (info) {
    ...
    this.Codigo(info.UserCode);
    this.Status(info.Status);
    this.IdSucursal = info.IdSucursal;
    this.IdEmpresa = info.IdEmpresa;
...
CatalogService.prototype.Update = function (info) {
    if (info.IdEmpresa == 0) info.IdEmpresa = 1;
    if (info.IdSucursal == 0) info.IdSucursal = 1;
...
Kernel.Configuration.Preferences = function () {
    var self = this;

    this.CallServiceMethod = function (method, params) {
        return $.ajax({ type: "POST", url: App.ServicePath + '/Kernel/Configuration/Preferences.svc/' + method, data: JSON.stringify(params), contentType: "application/json; charset=utf-8", dataType: "json",
            success: function (msg) {
            },
            error: function (e) {
            }
    ...
    this.GetPreference = function (idUsuario, path, key) {
        var params = { 'idUsuario': idUsuario, 'path': path, 'parameterName': key }
        return self.CallServiceMethod("GetPreference", params);
    }

    this.SavePreference = function (idUsuario, path, key, value) {
        var params = { 'idUsuario': idUsuario, 'path': path, 'parameterName': key, 'value': value }
        return self.CallServiceMethod("SavePreference", params);
    }
}
...
Kernel.Security.UsuarioService = function () {
    ...
    this.CargarAccesos = function (idUsuario) {
        var data = { 'sessionID': App.Session.SessionID, 'idUsuario': idUsuario }
        return self.CallServiceMethod("CargarAccesos", data);
    }

    this.GuardarAccesos = function (idUsuario, accesos) {
        var data = { 'sessionID': App.Session.SessionID, 'idUsuario': idUsuario, 'accesos': accesos }
        return self.CallServiceMethod("GuardarAccesos", data);            
    }
}
Kernel.Security.UsuarioService.prototype = new CatalogService({ WebService: 'Kernel/Usuario.svc' });
...
Kernel.Security.ModuloService = function () {
    ...
    this.LoadRootModules = function (idCategory) {
        var data = { 'sessionID': App.Session.SessionID, 'idCategory': idCategory }
        return self.CallServiceMethod("LoadRootModules", data);
    }

    this.LoadProcesos = function (idModulo) {
        var data = { 'sessionID': App.Session.SessionID, 'idModulo': idModulo }
        return self.CallServiceMethod("LoadProcesos", data);        
    }
